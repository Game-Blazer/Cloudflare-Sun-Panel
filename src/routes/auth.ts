...
import type { LoginRequest, RegisterRequest, ApiResponse, UserInfo } from '../models/types';
...
authApp.post('/login', async (c) => {
  ...
  const pwdValid = await verifyPassword(body.password, (user as any).password);
  if (!pwdValid) {
    return c.json({ code: 400, msg: '用户名或密码错误', data: null } satisfies ApiResponse);
  }
  ...
  // 更新 token 到数据库
  await db.prepare('UPDATE users SET token = ? WHERE id = ?').bind(token, (user as any).id).run();

  const userInfo: UserInfo = {
    id: (user as any).id,
    username: (user as any).username,
    name: (user as any).name || '',
    headImage: (user as any).head_image || '',
...
authApp.post('/register', async (c) => {
  ...
  await db.prepare('UPDATE users SET token = ? WHERE id = ?').bind(token, result.meta.last_row_id).run();

  // 创建默认用户配置
  await db.prepare('INSERT INTO user_configs (user_id) VALUES (?)').bind(result.meta.last_row_id).run();
